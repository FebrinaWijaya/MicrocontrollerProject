 ...
 * Author: febri
 *
 * Created on December 6, 2018, 3:18 PM
 */
...
//    PIR1bits.TMR2IF = 0;
    ...
    PR2 = 155; //timer 2 is compared to PR2=155 at each cycle
    CCPR1L = 4; //-90deg
    CCP1CONbits.DC1B1 = 0;
    CCP1CONbits.DC1B0 = 0;
...
void __interrupt () my_isr_routine (void)
{
    if(INTCONbits.INT0IF){
        INTCONbits.INT0IF = 0;
        INTCONbits.INT0IE = 0;
        if(CCPR1L == 0x04) //-90 to 0
        {
            //CCPR1L = 0x0A;      
            while(CCPR1L<18)
            {
                while(CCP1CONbits.DC1B<3)
                    CCP1CONbits.DC1B++;
                CCP1CONbits.DC1B = 0;
                CCPR1L++;
            }
        }
//        else if(CCPR1L == 0x0A) //0 to +90
//        {
//            //CCPR1L = 0x12;
//            while(CCPR1L<18)
//            {
        ...
        else if(CCPR1L == 0x12) //+90 to -90
        {
            ...
            while(CCPR1L>4)
            {
                while(CCP1CONbits.DC1B>0)
                    CCP1CONbits.DC1B--;
                CCPR1L--;
                if(CCPR1L>4) CCP1CONbits.DC1B = 3;
            }
        }
        INTCONbits.INT0IE = 1;
    }
